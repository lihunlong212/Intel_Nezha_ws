  ...
  ranges_precomputed_(false)
{
  ...
  map_x_max_m_          = declare_parameter("map_x_max_m",          1.5);
  map_y_min_m_          = declare_parameter("map_y_min_m",          0.25);
  map_y_max_m_          = declare_parameter("map_y_max_m",          1.5);
  ...
  // ── 多帧累积参数 ───────────────────────────────────────────
  accumulation_frames_   = declare_parameter("accumulation_frames",   10);
  cluster_merge_dist_m_  = declare_parameter("cluster_merge_dist_m",  0.20);
  min_votes_             = declare_parameter("min_votes",              5);
...
// ─────────────────────────────────────────────────────────────────────────────
// 预计算每个角度与柱子矩形区域相交时的最大允许距离。
// 使用通用射线-矩形求交，避免依赖雷达索引区间或固定的左右方向。
// ─────────────────────────────────────────────────────────────────────────────
void PillarDetectorNode::precomputeMaxRanges(const sensor_msgs::msg::LaserScan & scan)
{
  ...
  for (int i = 0; i < n; ++i) {
    ...
    double entry_range = 0.0;
    double exit_range = std::numeric_limits<double>::infinity();
    const auto clip_axis = [&entry_range, &exit_range](double direction, double min_bound,
      double max_bound) -> bool {
        if (std::fabs(direction) <= 1e-9) {
          return min_bound <= 0.0 && 0.0 <= max_bound;
        }
        double t0 = min_bound / direction;
        double t1 = max_bound / direction;
        if (t0 > t1) { std::swap(t0, t1); }
        entry_range = std::max(entry_range, t0);
        exit_range = std::min(exit_range, t1);
        return entry_range <= exit_range;
      };

    const bool intersects =
      clip_axis(cos_t, map_x_min_m_, map_x_max_m_) &&
      clip_axis(sin_t, map_y_min_m_, map_y_max_m_) &&
      exit_range >= 0.0;
    max_range_per_idx_[i] = intersects ? exit_range : 0.0;
  }
  ...
  const sensor_msgs::msg::LaserScan & scan)
{
  ...
  // 遍历整圈，实际有效区域由通用射线-矩形过滤决定。
  const int seg_start = 0;
  const int seg_end   = n;
  ...
  for (const auto & group : groups) {
    ...
    if (!too_close) {
      detections.push_back({cx, cy, frame_count_});
    }
  }
  ...
  for (int lbl = 0; lbl < next_label; ++lbl) {
    ...
    for (int i = 0; i < nd; ++i) {
      if (labels[i] == lbl) {
        ...
        sum_y += dets[i].y_m;
        ++count;
        if (std::find(source_frames.begin(), source_frames.end(), dets[i].frame_index) ==
          source_frames.end())
        {
          source_frames.push_back(dets[i].frame_index);
        }
      }
    }
    const int frame_votes = static_cast<int>(source_frames.size());
    if (frame_votes >= min_votes_) {
      clusters.push_back({sum_x / count, sum_y / count, frame_votes});
    }
  }
...
void PillarDetectorNode::publishPillars(const std::vector<Cluster> & pillars)
{
  ...
  // /detected_pillars is the route gate input: one lateral Y coordinate per pillar.
  // Exactly two detected pillars therefore produce exactly [y1, y2].
  std_msgs::msg::Float32MultiArray detected_msg;
  detected_msg.data.resize(sorted_pillars.size());

  // Keep the legacy full positions for any consumer that needs X/Y coordinates.
  std_msgs::msg::Float32MultiArray position_msg;
  position_msg.data.resize(sorted_pillars.size() * 2);
  for (std::size_t k = 0; k < sorted_pillars.size(); ++k) {
    detected_msg.data[k] = static_cast<float>(sorted_pillars[k].y_m);
    position_msg.data[k * 2] = static_cast<float>(sorted_pillars[k].x_m);
    position_msg.data[k * 2 + 1] = static_cast<float>(sorted_pillars[k].y_m);
  }
  ...
  for (std::size_t k = 0; k < sorted_pillars.size(); ++k) {
    ...
    RCLCPP_INFO(get_logger(), "║    y = %+.3f m  ( 左方 %.1f cm )       ║",
  ...
  RCLCPP_INFO(get_logger(), "║  /detected_pillars: [y1,y2,...] 单位:米  ║");
